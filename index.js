...
const token = '6741576591:AAHM2T2SCWDzXkd6Q83Z2gskQuJEZdvOhzw';
...
bot.on('message', (msg) => {
  ...
  if (command.startsWith('/mix')) {
    ...
    if (args.length === 5 && url && time && thread && rate) {
      ...
      exec(`node mix.js ${url} ${time} ${rate} ${thread}`, (error, stdout, stderr) => {
        if (error) {
      ...
      bot.sendMessage(chatId, 'Format pesan tidak benar. Gunakan format: /mix [url] [time] [rate] [thread]');
    }
  }
});
